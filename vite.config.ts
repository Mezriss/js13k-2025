...
function roadrollerPlugin(): Plugin {
  return {
    ...
    transformIndexHtml: async (
      ...
      const minifiedHtml = minifyHtml(cssInHtml, options);
...
async function embedJs(html: string, chunk: OutputChunk): Promise<string> {
  ...
  if (process.env.USE_RR_CONFIG) {
    try {
      options = JSON.parse(
        readFileSync(`${__dirname}/roadroller-config.json`, "utf-8"),
      );
    } catch (error) {
      throw new Error(
  ...
  await Promise.all([
    writeFileSync(`${path.join(__dirname, "dist")}/output.js`, htmlInJs),

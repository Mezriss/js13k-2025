import { SvgAsset } from "@/entities/svgAsset";
import { Vector2 } from "@/util/vector2";
import { screen } from "@/util/draw";

const eyeFill = ["#e9ce97", "#688a9a", "#eee", "#222"];

const eyeL = new SvgAsset(
  `M-14.9 12.4C-14.5 11.1-13.7 10.4-11.9 10.7-10 11.1-9.4 13.4-9.6 15-9.7 16.3-10 16.8-11.5 17.1-12.7 17.4-14 16.3-14.6 15.2-15.4 13.5-14.9 12.4-14.9 12.4Z
M-13.8 11.6C-13.2 11.3-11.9 10.9-11 12.6-10.1 14.2-10.2 16-11 16.4-11.8 16.9-12.8 16.5-13.3 16.1-14.6 15.2-14.9 14.2-14.8 13.2-14.6 12.1-13.8 11.6-13.8 11.6Z
M-14.3 12.9C-14 12.4-13.5 12.3-12.8 12.5-11.9 12.9-11.4 14-11.7 15-11.9 15.5-12.6 16-13.3 15.6-14 15.2-14.5 14.1-14.5 13.6-14.5 13.2-14.3 12.9-14.3 12.9Z
M-12.7 14C-12.6 14.7-12.7 14.9-13.1 15.1-13.3 15.2-14 15-14.2 14.4-14.5 13.7-14.4 13.3-14.3 13-14.1 12.6-13.6 12.7-13.6 12.7-13 13-12.8 13.5-12.7 14Z
`,
  [null, "#222", null, null],
  eyeFill,
);

const eyeR = new SvgAsset(
  `M10.3 12.2C10.3 12.2 11.1 9.6 13.6 10.2 15.3 10.6 14.7 13 13.8 14.2 13 15.4 12.3 16 11.1 15.7 9.4 15.3 10.3 12.2 10.3 12.2Z
M11.2 12.4C11.2 12.4 12.4 9.9 13.6 10.5 15.2 11.4 13.9 13.7 13.9 13.7 13.9 13.7 12.9 15.6 11.8 15.3 10 15 11.2 12.4 11.2 12.4Z
M12.2 13.6C12.2 13.6 11.8 11.6 13.5 11.2 14.9 10.8 14 13.4 13.7 13.8 13.5 14.1 13.3 14.3 12.8 14.2 12.2 14.1 12.2 13.6 12.2 13.6Z
M12.5 13.4C12.3 13.1 12.3 12 13.2 11.6 14 11.2 14.1 11.9 14.1 12.3 14.1 12.7 13.8 13.6 13.3 13.8 12.9 14 12.5 13.6 12.5 13.4Z
`,
  [null, "#222", null, null],
  eyeFill,
);

const body = new SvgAsset(
  `M15.5 0C15.5 0 16.9 6.9 16.3 11.1 15.8 14.8 13.4 21.9 9.1 24.2 7.2 25.2-.9 27.7-7.9 24.4-11.2 22.9-16 18.9-16.7 14.4-17.5 9.8-17.5-0-17.5-0Z
`,
  [null],
  ["#222"],
);

const mouth = new SvgAsset(
  `M-0 21.9C2 22 4 21.5 5.9 20.8 8 20 10.2 18.2 11 17.1 12 15.8 12.6 16.9 12.4 17.7 12.1 18.8 10.5 21.8 8.7 22.8 6.8 23.9 2.2 24.9 0 24.8-2.1 24.8-5.2 24.9-7.6 23.7-8.9 23-11.7 21.3-12.1 19.9-12.3 18.8-11.3 17.8-10.2 18.3-9 18.9-7.7 20.1-6.3 20.7-3.2 22-0 21.9-0 21.9Z
M-11.7 19.5C-11.9 19-11.5 18.6-10.9 18.8-10.2 19.1-7.8 21-6.6 21.5-4.8 22.2-1.4 23.2 2.8 22.5 7 21.7 10.9 18.2 11.1 17.8 11.5 17.2 12.3 17.2 11.6 18.5 10.9 19.8 8.5 22.2 7 22.9 5.5 23.6 2.7 24.3 0 24.3-2.7 24.3-6.1 23.9-7.3 23.3-11 21.5-11.7 19.5-11.7 19.5Z
M-11.4 19.6-10.6 19.4-10.6 20.7-9.6 20.3-9.5 21.4-8.9 21-9 21.9-8.1 21.5-8 22.4-7.1 21.9-7 22.7C-7 22.7-5.6 22.3-5.5 22.4-5.5 22.5-5.1 23.3-5.1 23.3L-4.2 22.7-3.9 23.5-2.9 23-2.4 23.8-1.7 23.2-1.2 23.8C-1.2 23.8-.8 23.1-.7 23.2-.6 23.3.1 23.9.1 23.9L.6 23.1 1.6 23.9 2 23 2.8 23.9 3 23.1 4 23.7 4 22.8 4.9 23.3 5.1 22.4 6.1 23 6.1 22.2 7.3 22.9 7.1 21.7
M8.6 21.4 8.6 20.6 9.2 20.9 9.3 20.1 10 20.3 9.9 19.4 10.6 19.7 11 18.3
`,
  [null, null, "#222", "#222"],
  ["#eba785", "#eee"],
);

const nostrilL = new SvgAsset(
  `M-2.4 20C-2.3 19.4-2 19.1-1.7 19.1-1.4 19-.9 19.4-1.1 19.9-1.3 20.5-1.5 20.5-1.9 20.4-2.5 20.3-2.4 20-2.4 20Z
M-2.1 20C-2.1 20-2 19.3-1.7 19.3-1.3 19.4-1.4 20.1-1.4 20.1
`,
  [null, "#222"],
  ["#e9ce97"],
);

const nostrilR = new SvgAsset(
  `M1.3 19.9C1.3 19.9 1.2 19.4 1.6 19.2 1.9 19.1 2.1 19.1 2.4 19.4 2.7 19.8 2.6 20 2.5 20.2 2.3 20.3 1.9 20.6 1.5 20.4 1.2 20.1 1.3 19.9 1.3 19.9Z
  M1.6 20.2C1.6 20.2 1.5 19.6 1.8 19.4 2.1 19.3 2.4 19.9 2.4 19.9
`,
  [null, "#222"],
  ["#e9ce97"],
);

const whiskers = new SvgAsset(
  `M4.5 18.7C4.5 18.7 4.7 18.6 5.1 18.6 5.4 18.6 5.7 18.8 5.7 19.3 5.7 20 5.3 21.6 7.2 22 8.7 22.3 9.4 21.1 10.4 20.6 11.5 20 12.7 19.9 13.6 20.3 14.6 20.7 14.5 21.5 15.4 21.8 16 22 16.7 22 16.7 22
M-4.1 19.4C-4.1 19.4-4.2 19.1-5.1 19.1-6 19.2-6.4 19.6-6.5 20.3-6.7 21-6 22.1-6.2 22.9-6.3 23.8-7 24-8.4 23.9-9.6 23.8-11.4 22.4-12.2 23.2-13 24-13.1 25.6-13.1 25.6
`,
  ["#222"],
  [],
);

export const drawOnamazu = (scale: number, position = new Vector2(0, -45)) => {
  const onamazuScale = new Vector2(1, 1).scale(scale);

  body.draw(position, onamazuScale);
  eyeL.draw(position, onamazuScale);
  eyeR.draw(position, onamazuScale);
  nostrilL.draw(position, onamazuScale);
  nostrilR.draw(position, onamazuScale);
  mouth.draw(position, onamazuScale);

  screen.ctx.strokeStyle = "#e9ce97";
  screen.ctx.lineWidth = screen.scale * 0.9 * scale;
  for (let i = 0; i < 2; i += 1) {
    screen.ctx.beginPath();
    whiskers.drawPath(i, position, onamazuScale);
    screen.ctx.stroke();
  }

  screen.ctx.strokeStyle = "#222";
  screen.ctx.lineWidth = screen.scale * 0.6 * scale;
  for (let i = 0; i < 2; i += 1) {
    screen.ctx.beginPath();
    whiskers.drawPath(i, position, onamazuScale);
    screen.ctx.stroke();
  }
};
